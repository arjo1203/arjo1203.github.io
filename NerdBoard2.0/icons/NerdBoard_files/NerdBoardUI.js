...
LineThickDot.onMouseDrag = function(event) {
    ...
    if(disRatio <= 0) {
        ...
        NerdBoard.penStroke = NerdBoard.penStrokeRange * disRatio;
...
NerdUI.data = {
    ...
    animateOut: function(delta, vector, icon) {
        ...
        if(icon.data.name == "PencilOptionsView") {
            this.scaleBig(2, {a: 2, b: 5}, vector, icon);
            icon.opacity = 1;
        }
        else {
    ...
    scaleBig: function(delta, thres, vector, icon) {
        var scale = Math.abs(vector.x / delta);
        if( thres.a < scale && scale < thres.b) {
            icon.data.scales.push(scale.toPrecision(2));
            icon.scale(scale);
        }
    },
...
PencilOptionsView.data = {
    ...
    setDest: function() {
        this.Destination = NerdUI.data.destPoint(220, -50);
    },
    scales: []
};
